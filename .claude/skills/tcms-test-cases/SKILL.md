...
description: >
  Run the tcms plugin `tcms-test-cases` stage (construction phase) in isolation, without
  advancing the main workflow. Packages `/aidlc --stage tcms-test-cases --single`:
  the engine emits one run-stage directive for tcms-test-cases and its gate, the
...
# AI-DLC Stage Runner — tcms-test-cases

Run the `tcms-test-cases` stage from the tcms plugin on its own. This is opt-in packaging over
`/aidlc --stage tcms-test-cases --single`; the same stage is always reachable via
...
1. Ask the engine for the single-stage directive:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts next --stage tcms-test-cases --single
   ```

   The engine emits one `run-stage` directive for `tcms-test-cases` (carrying the
...
2. When the stage's work is done, commit the single-stage record:
   ...
   ```bash
   bun .claude/tools/aidlc-orchestrate.ts report --single --stage tcms-test-cases --result completed
   ```
