...
export const Sidebar: React.FC = () => {
  ...
  const isAdmin = user?.role === 'Project_Admin' || user?.role === 'Platform_Admin';
  ...
  return (
    <div className="w-64 bg-white border-r border-gray-200 h-screen flex flex-col shrink-0">
      ...
      <div className="flex-1 overflow-y-auto py-4">
        <div className="px-4 text-xs font-semibold text-gray-400 mb-2 tracking-wider">核心工作區</div>
        ...
        {isAdmin && (
          <div className="mt-6">
            <div className="px-4 text-xs font-semibold text-gray-400 mb-2 tracking-wider">系統管理</div>
            <nav className="space-y-1 px-2">
              <NavLink 
                to="/admin" 
                className={({ isActive }) => 
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="text-sm font-extrabold text-gray-900 truncate">{user?.username || 'User'}</div>
              <div className="text-[10px] font-bold text-gray-500 truncate uppercase tracking-wider">{user?.role || 'Guest'}</div>
            </div>
          </div>
          <button 
