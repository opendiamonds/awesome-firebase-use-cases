...
部署資產同步義務（`schema_rbac.sql`／`DEPLOY.md` 同步更新，blocking）**內建於 (a) 與 (d) 的 Definition of Done**，不另立項 [Q4] [memory:M1]。
...
- **Must**：(a) 記錄、(b) 顯示、(c) 視覺標示、(d) 權限開通 — 四項缺一不可 [Q1]。
...
**Dependency-first** [Q2]：依賴鏈 (a) → (b) → (c) 依序交付，(d) 權限開通與依賴鏈無技術耦合、排序殿後。細部的 Bolt 切分與經濟排序留給 delivery-planning，本文件只固定依賴序。
