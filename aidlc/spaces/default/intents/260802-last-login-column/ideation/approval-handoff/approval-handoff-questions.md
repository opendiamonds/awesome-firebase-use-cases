...
## Revision 1（2026-08-11，因上游分頁修訂而同步）

**觸發來源**：scope-definition Revision 2 新增 Must 能力 (f)／PU-6；rough-mockups Revision 1 定案分頁形式與全域計數的處置。

**本站不新增問題** —— 本件為彙整站，其內容全部承接自上游已定案的決議，無獨立的待決事項。既有 Q1／Q2 的答案不改寫，GO 建議不變。

**修訂範圍**：`initiative-brief.md` 的六項能力同步、PU-6 依賴記載、Won't Have 邊界澄清、未承諾清單新增全域逾期計數、Assumptions 新增兩項；`decision-log.md` 新增「Revision 週期」六條決議。

**GO 建議為何不變**：新增能力不改變可行性判定 —— 分頁改動的是既有端點的回應契約與兩種佈局的呈現，不引入新服務、新依賴或新基礎設施（AD-5 維持成立）。
