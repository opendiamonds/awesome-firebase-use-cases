...
- **scope-document**、**intent-backlog**（`../scope-definition/`，**Revision 2**）：**六項** Must 能力、排除清單與 proto-unit 依賴序。
...
scope-document（**Revision 2**）確立**六項** Must，一起上線才算完成；依賴序見 intent-backlog：
...
| PU-6 | **使用者清單分頁**（Revision 2 新增） | **無技術依賴**（清單端點為既有功能）；但**必須在 PU-5 之前定案分頁互動** —— 屬避免重工的排序約束，非技術依賴 |
| PU-4 | `Security_Reviewer` 檢視權限開通 | 無技術依賴，平行、排序殿後 |

**Won't Have**：登入／活動歷史紀錄、門檻 N 可設定介面、欄位級權限控制、依時間排序／篩選（**Revision 2 明確保留** —— 新增的 PU-6 分頁是本次唯一新增的清單互動，不連帶解除排序與篩選的排除）。**未承諾**：稽核報表匯出；**全域逾期計數**（rough-mockups Revision 1 評估後判定本輪不採用、不畫入線框，處置比照稽核報表匯出的未承諾先例）。
...
**GO** [Q2] — conditional GO 四前提已全數落地、風險均有處置、**六項** Must 依賴序已定；核可本 brief 即交接進 Inception（8 站、每站各一個 approval gate）。

> **Revision 1（2026-08-11）**：因 scope-definition Revision 2 新增 Must 能力 (f)／PU-6（使用者清單分頁）而修訂。本件的 **GO 建議不變** —— 新增能力不改變可行性判定：分頁改動的是既有端點的回應契約與兩種佈局的呈現，不引入新服務、新依賴或新基礎設施（AD-5 維持成立）。修訂範圍限於六項能力的同步、PU-6 的依賴記載、Won't Have 的邊界澄清與未承諾清單。

## Assumptions & Open Questions

- [assumption] （**Revision 1 更新**）上表未決項為 ideation 已知的完整清單 [Q1]；**Revision 1 另新增兩項**：PU-6 的分頁形式（頁碼式已定，每頁筆數／envelope 形式留 application-design）、以及分頁與逾期標示的互動（逾期帳號散落多頁時是否需跨頁彙總 —— rough-mockups 已判定本輪不採用全域計數）。若 inception 發現新未決項，依協定於該 stage 記錄，不回改本件
