...
> **本文件為 iteration 3 + Revision 1**。Revision 1（2026-08-09）新增 C-8 契約，並更新 C-6 的型別來源。
...
**型別來源（Revision 1）**：本元件使用的使用者物件型別**改採 C-8 產生的型別**，不再手寫。這使「後端改欄位名或改型別」在前端建置時失敗，而非執行期靜默變成 `undefined`。

**正規化契約仍然必要**（iteration 1 Finding 10）：產生的型別描述的是**後端已部署後**的形狀；前端先上時欄位仍可能不存在，且既有的資料抓取不做欄位驗證或白名單。因此本元件仍必須在傳遞點收斂：
...
**本元件為 Revision 1 新增**（`decisions.md` AD-9 的承載元件）。它完全在建置期，**不提供任何執行期介面**，因此本節定的是**產物契約**而非函式簽章。

### 產物契約（Q6=A 定案）

| 產物 | 位置 | 契約 |
|---|---|---|
| 規格 dump 腳本 | 後端 | 由程式碼取得規格，**不需啟動服務、不需資料庫、不需環境變數**（本站實測確認） |
| 規格檔 | **repo 根目錄** | 與程式碼一起 commit；**不得置於 `frontend/public/` 或任何被靜態服務原樣供出的路徑** |
| 產生的型別檔 | **`frontend/src/` 內，且 commit** | 前端 import 的對象；`tsc -b` 對它檢查 |
| **規格檔**漂移檢查 | CI **backend** job | 重新 dump 並與 commit 的規格檔比對 |
| **型別檔**漂移檢查 | CI **frontend** job | 重產型別並與 commit 的型別檔比對（該 job 有 node 與產生器，且為全樹 checkout） |

### 四項契約要求

1. **規格取得不得依賴執行中的服務。** CI 的 frontend job 不運行後端（實測確認），違反即 CI 不可行。
2. **漂移檢查必須能失敗**，且**不得在無關變更上失敗**。後者是第 4 項的來源。
3. **採用範圍限於使用者物件型別**（Q5=A）。其餘 51 處資料抓取的手寫型別不動，且**需明確記載哪些已遷移**。
4. **產生器只得輸出型別宣告，不得輸出任何進入 bundle 的執行期程式碼**（API client、fetch wrapper、執行期驗證器）。

### 為何需要第 4 項（Revision 1 reviewer Finding 4）

「不進入生產環境的執行路徑」是 AD-9 得以成為 AD-5 **窄**例外的**全部依據** —— `services.md` 的「運行特性與失敗模式各節不受影響」也建立在它之上。但前三項契約守不住它：一個產生 runtime client 或執行期驗證器的產生器**三項全部滿足**，卻直接推翻該不變式。

**可核對的形式**：產出物僅為型別宣告（`.d.ts` 或 type-only 匯出），不含任何會被打包進 bundle 的值。

### 型別產生不進 `npm run build`（Critical 1）

型別產生只在**開發與 CI** 執行；產生的型別檔 commit。理由：前端映像的 build context 只有 `frontend/`，規格檔在 repo 根目錄不在其中；若把產生步驟放進 `npm run build`，映像建置會因讀不到規格檔而失敗（CI docker-build gate + staging 部署 + 測試 stack 三處同時壞）。

commit 型別檔**不減損**前端側的保護 —— `tsc -b` 對它檢查前端用法。**但它引入了新的時效性問題**：`tsc -b` 檢查的是「用法對型別檔」，不是「型別檔對規格檔」，因此型別檔過期時它會順利通過（Revision 1 驗證輪 V-1）。這正是型別檔需要**自己的一道 gate**（frontend job 的漂移檢查）的原因；只靠開發者自律等同退回 AD-9 已否決的方案 A 形狀。

### 版本釘選（Critical 2，Q7=A）

**web 框架與資料驗證函式庫必須以精確等值形式釘住版本**（相容釋出形式仍會在次版本線上浮動，而觀測到的 20 行差異正是跨次版本產生的 —— 形式選錯等於沒釘）。**升版這兩支時，必須在同一個 PR 內重新 dump 規格檔並一併 commit**；屆時 gate 變紅是正確行為而非誤報。實測：規格輸出在同一組依賴版本下位元決定性（五組雜湊種子 sha256 相同），但跨版本會產生 20 行差異。後端依賴清單 12 行、版本約束 0 行、無 lockfile，CI 每次重新解析最新版 —— 不釘住則此 gate 必然在無關 PR 上變紅。

這是「全 repo 未 pin」的局部例外，理由記於 AD-9。

### 型別檔穿過的既有 gate

型別檔進入 `frontend/src/` 後會落入既有 lint 的作用域（忽略清單只有建置輸出目錄）。**採用時必須定案**：納入 lint 作用域（產生器輸出須自然通過現行規則集），或明文加入忽略清單（須記明為刻意的產出物豁免）。

### 依賴清單同步

新增 devDependency 時**必須一併重產並 commit `package-lock.json`** —— CI 用 `npm ci`，lock 與宣告不一致即硬失敗。

### 與其他元件的關係

- **C-4（序列化）** —— 規格內容來自它定義的回應模型。C-8 不改變回應形狀。
- **C-6（管理頁）** —— 唯一採用點。
- **其餘元件** —— 無關係。C-8 不進入執行路徑。
...
| C-5 | 五狀態呈現；不自算逾期 | 無 |
| C-6 | 傳遞前正規化為 C-5 的宣告型別 | C-4（HTTP）、C-5、**C-8（型別，建置期）** |
| C-7 | 只更新不插入；在權限種子之後執行；條件式套用 | 無（與 C-3 分屬不同資料表） |
| C-8 | 規格不依賴執行中的服務；**兩份產出物各有一道 gate**；不在無關變更上失敗（精確釘選）；採用範圍限一處；**只輸出型別、不輸出執行期程式碼** | C-4（規格內容來源，建置期非執行期） |

---

## Revision 1（2026-08-11）— C-9 使用者清單分頁

> 形狀宣告，非實作程式碼。識別字使用專案既有的命名慣例（Python `snake_case`、React `PascalCase` 元件／`camelCase` 變數）。

### 後端：清單端點的分頁契約

| 項目 | 形狀 | 約束來源 |
|---|---|---|
| 查詢參數 `page` | 整數，**最小值 1**，預設 1 | AD-11（框架原生約束）、AC-5.5 |
| 查詢參數 `page_size` | 整數，**最小值 1、最大值 100**，預設 **20** | AD-11、AD-10、NFR-8 |
| 回應模型 `UserListPage` | `items: List[UserSchema]`、`total: int`、`page: int`、`page_size: int` —— **四欄皆必填、皆無預設值** | AD-10、AC-5.2 |
| 查詢 | `ORDER BY id` → `OFFSET (page-1)*page_size` → `LIMIT page_size`；`total` 由不帶 offset／limit 的計數查詢取得 | AC-5.3、FR-6.2 |
| 超出範圍 | 不特別處理 —— offset 超過總數時查詢自然回空清單，`page` 照樣回顯請求值 | FR-6.4 |

**為何四欄皆無預設值**：`UserListPage` 是本 intent 的**第四個回應構造點**。若 `page: int = 1` 這樣宣告，一個完全沒讀查詢參數的實作也會輸出三個正確形狀的 key，而 AC-5.2 的值斷言正是為了分辨這種自我回報式實作。無預設值使「忘記傳」在構造當下就是錯誤，而非靜默通過。

**`total` 必須是獨立的計數查詢**，不得由 `len(items)` 導出 —— 後者對「總筆數 = 本頁筆數」的實作恆真，AC-5.2 的第一個 Then 正是為此。

### 前端：`PaginationControl`

Props 契約與六個狀態見 `refined-mockups/interaction-spec.md`（該檔為互動規格的單一來源，此處不重述以免兩份漂移）。本檔只補設計層的兩個約束：

| 約束 | 內容 | 理由 |
|---|---|---|
| 總頁數為導出值 | 由 `total` 與 `pageSize` 計算，**不另設 prop、不另存 state** | `team.md ## Code Style` 的單一真實來源規則；兩份來源會漂移 |
| `pageSize` 取自回應而非前端常數 | 後端是每頁筆數的真相來源 | 前端常數與後端預設值會各自演化，形成第二份物化（同一條規則） |

### 前端：C-6 的分頁狀態與抓取

| 項目 | 形狀 | 約束來源 |
|---|---|---|
| 分頁狀態 | 單一 `page` state（1 起算）；`total`／`pageSize` 直接取自最近一次回應，**不另存** | 單一真實來源 |
| 抓取函式 | 既有的純抓取函式增加 `page` 參數，**回傳整個 envelope**（不在函式內拆解） | 既有三層拆分形狀（C-6 既有約束） |
| 逾期旗標正規化 | **收斂在抓取函式內**，三個呼叫端（初次載入、`fetchUsers`、切頁）皆不各自處理 | AC-5.8；切頁是第三個呼叫點，分散處理必漏 |
| 切頁 | `onPageChange` → 設定 `page` state → 既有的抓取路徑；**忙碌狀態與初次載入的 `isLoading` 分離**，使分頁控制不落入被整塊替換的容器 | AC-5.10、refined-mockups 定案 5／9 |
| 刪除後 | 就地移除該列 ＋ `total` 本地遞減 → 以目前 `page` **背景**重抓該頁 | AD-12 |
| 刪除後重抓的畫面行為 | **不進入任何載入態** —— 不設 `isLoading`、不設 `isBusy`、**不得沿用既有的 `fetchUsers()`**（它會 `setIsLoading(true)` 把整個容器替換成載入畫面，每刪一列閃一次）。回應抵達時靜默替換 `items` 與三個分頁值 | AD-12「與載入狀態機的關係」 |
| 刪除後重抓失敗 | 保留就地移除後的畫面（使用者看到的仍是正確結果），以既有 toast 提示同步失敗；**不回滾、不跳錯誤畫面** | 同上 |
| 三種抓取路徑的旗標 | 初次載入＝既有 `isLoading`（整塊替換）；切換頁次＝新設 `isBusy`（控制項留在畫面上）；刪除後重抓＝**無旗標**。三者互不共用 | 同上 |
| 重疊重抓的併發保護 | 只有「最後發出」的重抓回應能寫入 state —— 以遞增請求序號或 `AbortController` 擇一實作。既有的 `cancelled` flag 只防卸載後 setState，不防亂序返回 | AD-12「併發保護」 |
| 啟停用／角色調整後 | 純就地更新，**不重抓** | AC-5.6、AD-12 |
| 失敗路徑 | `catch` **不得**呼叫整份重抓（現行 `AdminPage.tsx:91-94` 會，是 US-5 之後唯一殘留的整份重抓點） | AC-5.6 最後一個 And |
