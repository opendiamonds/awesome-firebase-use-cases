...
> **本文件為 iteration 3 + Revision 1**。iteration 1〜2 的審查紀錄保留於文末；**Revision 1（2026-08-09）**由 units-generation Q2=B 觸發回跳，新增元件 **C-8**（API 型別契約產生），並更新 C-6 的正規化契約。舊版已歸檔於 `<record>/archive/2026-08-09-application-design/`。
...
| C-8 | API 型別契約產生 | 跨層 · 建置期 | 新增（Revision 1） | 規格 dump、committed spec、前端型別產生、CI 漂移檢查 |
...
**正規化責任**（iteration 1 Finding 10；Revision 1 更新）：C-5 的兩個輸入皆為必填且型別不含未定義。本元件必須在傳遞點把未定義收斂為 C-5 宣告的型別（空值與否），使「部署順序無硬性約束」的論證成立。

**型別來源改為產生（Revision 1）**：本元件所使用的使用者物件型別**改採 C-8 產生的型別**，不再是手寫的本地 interface。這給了「後端改欄位名或改型別」編譯期保護 —— 前端建置會失敗，而非在執行期靜默變成未定義。正規化責任不因此消失：產生的型別描述的是**後端已部署後**的形狀，前端先上時欄位仍可能不存在，故收斂仍必要。

**範圍限縮**（本站 Q5=A）：只有使用者物件型別改採產生型別；本頁其餘資料來源與前端其他 51 處資料抓取的手寫型別**不動**。
...
> **本元件為 Revision 1 新增**（`decisions.md` AD-9 的承載元件，AD-5「不新增外部依賴」的具名例外）。首版經 reviewer 判定 NOT-READY（2 Critical），本節為修正後版本。

**職責**：讓後端的回應形狀與前端的型別之間有一條**編譯期會失敗**的連結。

### 四個組成（Q6=A 定案後的形狀）

| 組成 | 位置 | 做什麼 | 是否 commit |
|---|---|---|---|
| 規格 dump 腳本 | 後端 | 由程式碼取得 API 規格，輸出為 repo 根目錄的規格檔 | 是（腳本） |
| 規格檔 | **repo 根目錄**（與既有的 schema 檔、部署文件同層） | 型別產生的輸入 | 是 |
| 產生的型別檔 | `frontend/src/` 內 | 前端實際 import 的型別 | **是** |
| **規格檔**漂移檢查 | CI **backend** job | 重新 dump 並與 commit 的規格檔比對 | — |
| **型別檔**漂移檢查 | CI **frontend** job | 重產型別並與 commit 的型別檔比對 | — |

### 為何型別檔要 commit、且型別產生不進 `npm run build`（Critical 1 修正）

首版把型別產生放進 `npm run build`，並在 `services.md` 把 `docker-build` 記為「無變更」。**兩者都錯，且合起來會壞掉**：

實測確認前端映像的 build context 三處皆為 `frontend/`（CI 的 docker-build、staging 部署編排、測試用編排），而 `frontend/Dockerfile` 是 `COPY . .` + `RUN npm run build`。規格檔由**後端**腳本產生，其自然落點（repo 根目錄）**不在該 context 內** —— 映像建置時讀不到規格檔，`npm run build` 失敗。後果是 CI 第四道 gate 紅燈、staging 部署失敗（並觸發 rollback job 開 revert PR）、`ui-regression` 的短生命週期 stack 起不來。

**定案（Q6=A）**：型別產生**不進 `npm run build`**，只在開發與 CI 執行；產生的型別檔 **commit 進 `frontend/src/`**。於是：

- Docker 建置只需要既有的 `frontend/` 內容（含 commit 的型別檔）→ **`docker-build` 真的不受影響**
- 編譯期保護不減 —— `tsc -b` 對 commit 的型別檔檢查前端用法。**但這只涵蓋「用法對型別檔」，不涵蓋「型別檔對規格檔」**（見下方 §兩份產出物需要兩道 gate）
- 規格檔留在 repo 根目錄，**不進 `dist/`**

### 兩份產出物需要兩道 gate（Revision 1 驗證輪 V-1）

Q6=A 把型別檔從「每次建置重產」改成 **committed 產出物**，於是 repo 內有**兩份**需要維持時效的產出物。首版只為規格檔定了 gate，留下一條靜默失敗路徑：

> 後端改欄位名 → 開發者重新 dump 規格（backend job 紅燈逼他做）→ **忘了重跑型別產生** → 型別檔仍宣告舊欄位名 → `tsc -b` 對舊型別檢查前端用法，**通過** → 前端執行期靜默取得未定義值

**`tsc -b` 結構上抓不到這條** —— 它檢查的是「用法是否符合型別檔」，不是「型別檔是否符合規格檔」。若只靠開發者自律，就退回成 AD-9 已否決的方案 A 形狀（人工對齊，無自動攔截）。

**定案：兩道 gate 分屬兩個 job。**

| 產出物 | Gate 位置 | 為什麼是那裡 |
|---|---|---|
| 規格檔 | CI **backend** job | 真相來源是後端程式碼；該 job 已有 Python 環境 |
| 型別檔 | CI **frontend** job | 需要 node 與產生器 devDependency；該 job 為全樹 checkout，讀得到 repo 根的規格檔 |

**backend job 沒有 node，結構上不可能重產型別檔** —— 首版把型別檔的攔截點記在 backend job 是錯的，已更正。

### 為何規格檔不得落在會被靜態服務供出的目錄（Major 3）

實測：`frontend/nginx.conf` 為 `root /usr/share/nginx/html` + `try_files $uri`，而 Vite 會把 `frontend/public/` **原樣複製**進 `dist/`。規格檔若落在那裡，`https://<公開網域>/<規格檔>` 會把 36 個 path、28 個 schema 的完整 API 地圖（含全部使用者管理與權限端點）對**未認證訪客**公開，且 nginx 已對 JSON 開 gzip。

目前後端自帶的規格端點在公網不可達（nginx 只反向代理 `/api/`），**本元件不得破壞這個既有狀態**。

**契約**：規格檔與型別檔**皆不得置於 `frontend/public/` 或任何會被靜態服務原樣供出的路徑**。Q6=A 的形狀天然滿足此約束（規格檔在 repo 根、型別檔在 `src/` 內經編譯）。

### 為何規格來自檔案而非 live 端點

實測確認 CI 的 frontend job 是獨立的，**後端不會在該 job 中運行**。這不是偏好問題 —— 打 live 端點在現行 CI 結構下不可行。

**規格可在不啟動服務、不連資料庫的前提下由程式碼取得**：本站實測確認（36 個 path、28 個 schema、OpenAPI 3.1.0，約 66 KB），且連環境變數都非必要（資料庫連線字串與 JWT 密鑰皆有預設值，連線引擎為惰性建立）。
（首版把此結論的論據寫成「CI backend job 的 import smoke 已證實」，**論據不成立** —— import smoke 只跑 `import main`，從未呼叫規格產生。結論為真但需以本站實測為據，不得引用 import smoke。）

### 漂移檢查的穩定性（Critical 2 修正）

首版只把「忘了重新 dump 就是 CI 紅燈」列為代價，**漏了「沒忘記也會紅燈」**。

實測結果分兩層，方向與直覺相反：

| 層面 | 實測 | 結論 |
|---|---|---|
| 同一組依賴版本 | 五組不同的雜湊種子各 dump 一次，sha256 **完全相同** | **位元決定性**，非每次執行都飄 |
| 跨依賴版本 | 同一份原始碼，僅更換 web 框架與資料驗證函式庫的版本 → **20 行差異**、sha256 改變 | **會飄** |

而後端依賴清單**12 行、版本約束 0 行、無 lockfile**，CI 每次安裝都重新解析最新版。因此這道 gate 會在**完全無關的 PR** 上變紅，且該紅燈與「真的漂移了」在訊號上不可區分。

**定案（Q7=A）**：把 **web 框架與資料驗證函式庫釘住版本**列為本元件的契約要求。**釘選必須為精確等值形式** —— 以相容釋出形式釘在次版本線上仍會浮動，而上述 20 行差異正是跨次版本產生的，形式選錯等於沒釘（Revision 1 驗證輪 V-4）。**升版這兩支依賴時，必須在同一個 PR 內重新 dump 規格檔並一併 commit** —— 屆時 gate 變紅是正確行為而非誤報。

**釘選範圍經實測足夠，不需擴大**（Revision 1 驗證輪）：底層 ASGI 框架雖然真的在浮動（上游只宣告下限、無上限），但跨主版本的輸出**位元相同**；型別輔助套件四支全降到宣告最低版、Python 由 3.13 換到 CI 使用的 3.12，輸出同樣位元相同。資料驗證函式庫對其原生核心是精確釘選，不需另外釘。本 app 的請求／回應模型未使用電子郵件型別、表單或檔案上傳，故 extras 中兩個會影響 schema 的可選套件在本 app 上不成立。這形成「全 repo 未 pin」的局部例外，理由記於 `decisions.md` AD-9；它同時是對 team-practices 已記載的既有風險（三處各自解析最新版、可能彼此不同）的局部改善。

### 型別檔穿過的既有 gate（Revision 1 驗證輪 V-5）

型別檔進入 `frontend/src/` 之後會落入既有 lint 的作用域（忽略清單只有建置輸出目錄，套用範圍涵蓋該目錄下所有 TypeScript 檔）。**採用時必須定案**：型別檔納入 lint 作用域（則產生器輸出必須自然通過現行規則集），或明文加入忽略清單（則需記明這是刻意的產出物豁免）。

失敗大聲不靜默（lint 紅燈立即可見），故不阻擋設計，但必須在採用時決定而非發現時才處理。

### 邊界與所有權

- **擁有**：規格檔與型別檔的產生與時效性（**各由一道 gate 承載**，見下）、上述兩項位置約束、版本釘選
- **不擁有**：回應的欄位內容（屬 C-4）、前端如何使用型別（屬 C-6）、任何執行期行為

### 採用範圍

產生器涵蓋整份規格，但**只有使用者物件型別實際被前端採用**（Q5=A）；其餘 51 處資料抓取維持手寫型別。reviewer 實測確認此限縮**無衝突風險**：使用者回應模型與前端既有的本地型別欄位 1:1 對應，且三個使用者物件端點皆宣告了回應模型，型別確實產得出來。

### 已知代價（如實記載）

- 本 intent **唯一帶有工具鏈黏性的變更**（AD-9 可逆性為「中等」，其餘八個 ADR 皆為「易於變更」）
- **兩份** committed 產出物（規格檔 + 型別檔），規格檔約 66 KB、涵蓋 36 個 path —— 每次後端 API 變更都會出現在 PR diff 中
- 規格檔忘了重新 dump 就是 CI 紅燈（刻意的攔截點，但增加 PR 往返）
- repo 內會同時存在產生型別與手寫型別兩種形狀，需明確記載哪些已遷移
- 新增的 devDependency 需一併重產並 commit `package-lock.json`（CI 用 `npm ci`，lock 與宣告不一致即硬失敗）
...
| NFR-3 安全（ADR-0006 四面向） | **C-7**（IAM 面向）、**C-8**（network exposure 面向） | 本站有兩個 security 落點：權限變更（C-7）與 C-8 的規格檔位置契約（見 AD-9 的四面向判定表） |
...
| units-generation 的單元間整合契約 | **C-8**（Revision 1） |
...
## Review — Revision 1 (final)

**Reviewer**: aidlc-architecture-reviewer-agent · Revision 1 最終驗收
**Date**: 2026-08-09T06:21:59Z
**審查範圍**：僅逐條驗收「Revision 1 (verify)」段落列出的 V-1〜V-5，並確認修訂本身未引入新的不一致。不重跑 Critical 1／Critical 2 的實測，不重審 C-1〜C-7、AD-1〜AD-8，不提新設計建議。
**Verdict**: **READY**
...
| # | 原嚴重度 | 判定 | 說明 |
|---|---|---|---|
| V-1 | Major | **已修正** | 三處假陳述與一處直接矛盾已全部更正，且四份 artifact 現在互相一致：①`services.md` L143-144 已把「規格檔未隨程式碼更新 → CI backend job 紅燈」與「型別檔未隨規格檔更新 → CI **frontend** job 紅燈」拆成兩句分屬正確的 job，不再是原本混寫的「規格檔或型別檔 → backend job 紅燈」；②`components.md` L374 的 C-8「擁有」清單現寫「規格檔與型別檔的產生與時效性（**各由一道 gate 承載，見下**）」——型別檔的時效性現在確實有承載，宣稱與事實一致；③`components.md` L314 與 `component-methods.md` L359 的「編譯期保護不減」都已加上限定句（「但這只涵蓋『用法對型別檔』，不涵蓋『型別檔對規格檔』」／「但它引入了新的時效性問題」），不再是無保留的宣稱。新增的 gate 本身也已落地：`services.md` L130 的 CI 影響表 frontend 列現寫「新增一道**型別檔漂移檢查**（重產型別並與 commit 的型別檔比對）」，與 `components.md` L295-303、L317-332（「兩份產出物需要兩道 gate」新節）、`component-methods.md` L332-340（產物契約表新增型別檔漂移檢查列）三處逐字對齊，backend／frontend 分工一致。全樹掃描確認無殘留的「backend job 攔截型別檔」活性宣稱（僅審查歷史記錄與「這是錯的，已更正」的說明性引用殘留，符合預期，不回改） |
| V-2 | Minor | **已修正** | `component-dependency.md`：①L57 邊數敘述已改為「十五條依賴（其中**四條為建置期**，圖上以虛線表示）」，與圖上 11 條實線＋4 條虛線一致；②L105 通訊模式表已改為「C-4 → C-8、C-8 → 規格檔、**規格檔 → 型別檔**、**型別檔 → C-6**」共四條邊，且註記「生產環境跑起來後這**四條**邊都不存在」，已不存在的「規格檔 → C-6」邊已移除（全檔掃描確認零殘留）；③L55 文字 fallback 開頭已改為「分**五層**」，與圖上五個 subgraph（FE／REQ／BOOT／DATA／BUILD）一致。三處連帶敘述全數同步，無新增不一致 |
| V-3 | Minor | **已修正** | `services.md`：①L27 既有服務拓樸表前端列已改為「管理頁新增一欄與小螢幕卡片佈局；**新增一份 commit 的型別檔**」，變更性質改為「既有頁面擴充；`npm run build` 流程本身不變」，不再是首版「建置新增型別產生步驟」的舊框架，且與同檔 L130「`npm run build` 本身不變」互相印證；②L29「建置與 CI」列已補上型別檔，現寫「repo 新增**兩份** committed 產出物（規格檔、型別檔）」，不再遺漏。`components.md` NFR-3 列（現於元件對應表）已寫「**C-7**（IAM 面向）、**C-8**（network exposure 面向）」並指向 `decisions.md` AD-9 的四面向判定表，第二個 security 落點不再對只讀該表的人隱形 |
| V-4 | Minor | **已修正** | `components.md` L362 與 `component-methods.md` L363 皆已補上「釘選必須為精確等值形式 —— 以相容釋出形式釘在次版本線上仍會浮動」，以及「升版這兩支依賴時，必須在同一個 PR 內重新 dump 規格檔並一併 commit」，兩處文字實質一致。`decisions.md` L381（Consequences 條列）未逐字複述這兩句，但也未與之矛盾——它是 ADR 的後果摘要而非契約本體，契約的權威落點（`components.md`／`component-methods.md`）已具體、可核對，不構成新的不一致 |
| V-5 | Minor | **已修正** | `components.md` L368-370 與 `component-methods.md` L369 皆新增「型別檔進入 `frontend/src/` 之後會落入既有 lint 的作用域」的段落，並要求「採用時必須定案：納入 lint 作用域（則產生器輸出須自然通過現行規則集）或明文加入忽略清單（則需記明為刻意的產出物豁免）」，且註明「失敗大聲不靜默，不阻擋設計，但必須在採用時決定」。兩處文字一致，且已列入 Revision 1 驗證輪的「帶入 Construction 的殘留事項」第 9 項 |

### Summary

V-1〜V-5 全數修正到位，且逐條回頭核對四份 artifact（`components.md`、`services.md`、`component-methods.md`、`component-dependency.md`）確認修訂本身沒有製造新的計數錯誤、殘留舊稱或矛盾陳述——特別是 V-1（原 Major）不再有任何一處對「型別檔時效性由誰攔截」給出無法兌現的承諾，四份文件對 gate 歸屬（frontend job）的敘述完全一致。全樹 grep 未發現「十四條」「分四層」「規格檔 → C-6」「backend job 攔截型別檔」等應被取代的舊措辭殘留。唯一不完全逐字對齊之處是 `decisions.md` L381 未重複 V-4 新增的「精確等值形式」與「同一 PR 內重新 dump」兩句，但該段落是 ADR 的 Consequences 摘要而非契約本體，契約權威落點已寫清楚且無矛盾，不構成新問題。判定 **READY**，開發者可依現行五份文件實作，不需再回頭問架構師。

---

## Review — Revision 1 (verify)

**Reviewer**: aidlc-architecture-reviewer-agent · Revision 1 驗證輪
**Date**: 2026-08-09T06:14:44Z
**審查範圍**：僅逐條驗收 Revision 1 首版的 8 條 findings，以及確認修訂未引入新問題。iteration 1〜4 已驗收的 C-1〜C-7、AD-1〜AD-8 不重審。
**Verdict**: **READY**（0 Critical、1 Major、4 Minor）

**結論先行**：兩個 Critical 的定案（Q6=A、Q7=A）經回 repo 與實跑驗證後**都站得住**——Q6=A 的 docker-build 推論逐段成立（`.dockerignore` 不排除 `src/`），Q7=A 的釘選範圍經實測**足夠**（starlette、typing-extensions、annotated-*、Python 版本全部無影響，見下方實測表）。但 Q6=A 的形狀**新開了一個洞**：型別檔改為 committed 之後，沒有任何機制保證它與規格檔同步，而三處主張聲稱有。見新發現 V-1。

### 8 條 findings 驗收

| # | 原嚴重度 | 判定 | 說明 |
|---|---|---|---|
| 1 | **Critical** | **已修正，推論成立** | Q6=A 的形狀經逐段查證：①`frontend/.dockerignore` 的排除清單為 `node_modules/`／`dist/`／`.env*`／`tests/`／`playwright.config.ts`／報表檔，**無任何 `src/` 下的路徑**，故 commit 在 `frontend/src/` 的型別檔會被 `COPY . .` 帶進映像；②三處 build context 複核無誤（`ci.yml` `context: ./frontend`、`deploy/docker-compose.deploy.yml:53` 與 `docker-compose.test.yml:46` 皆 `context: ../frontend`）；③規格檔在 repo 根目錄確實不在 context 內，但 Q6=A 之後**也確實不需要**——`npm run build` 不再讀它。另補查兩條原 finding 未列的路徑：`lint-fix` agentic workflow 亦跑 `npm run build`，但它是 repo 全樹 checkout，不受影響；`ui-regression` 經 `docker compose -f deploy/docker-compose.test.yml up -d --build` 走同一個 frontend context，已涵蓋。`services.md` L132 的 `docker-build`「無」已附 Q6=A 的成立條件 |
| 2 | **Critical** | **已修正，且釘選範圍經實測足夠** | 見下方「Critical 2 釘選充分性實測」。`fastapi` + `pydantic` 兩支確實足以消除跨版本漂移：`pydantic` 對 `pydantic-core` 是**精確釘選**（`pydantic-core==2.46.4`，metadata 實查），其餘浮動依賴實測對輸出零影響。AD-9 Consequences（L381）、`services.md` L140、`component-methods.md` L360-364 三處都已記載此失敗模式與處置。**殘留兩項未寫明的實作細節見 V-4（Minor）** |
| 3 | **Major** | **已修正** | `decisions.md` L401-410 新增 ADR-0006 四面向逐項判定表，**四項齊全、不適用者皆附理由**（IAM／Encryption／Audit logging 各有具體不適用理由，非留白）。network exposure 的處置經實測**確實擋住風險**：`frontend/nginx.conf` 僅 `location /api/` 反向代理 + `location /` 的 `try_files`，而 `main.py:24` 的 `FastAPI(title=...)` 用預設 `openapi_url="/openapi.json"`、全部 router 掛在 `/api/*` 之下，故後端自帶規格端點在公網確實不可達；規格檔置於 repo 根目錄則同時不在 frontend 與 backend 兩個映像的 context 內（backend context 為 `./backend`），`frontend/public/` 現況僅 `favicon.svg`／`icons.svg` 兩檔，明文禁止落在該目錄的契約已寫入 `components.md` L322 與 `decisions.md` L407。**殘留一處未同步見 V-3（Minor）** |
| 4 | **Major** | **已修正** | 第 4 項契約要求已寫入 `component-methods.md` L346，並在 L378-389 的介面契約摘要 C-8 列同步（「只輸出型別、不輸出執行期程式碼」）。可核對形式為「產出物僅為型別宣告（`.d.ts` 或 type-only 匯出），不含任何會被打包進 bundle 的值」——這是二元可判的，且與 `frontend/tsconfig.app.json` 的 `verbatimModuleSyntax: true`／`erasableSyntaxOnly: true` 相容（型別匯入必須顯式標示，值匯入藏不住）。L348-352 亦保留了「為何前三項守不住此不變式」的論證。`package-lock.json` 同步要求已補於 L366-368 與 `components.md` L361 |
| 5 | Minor | **已修正** | `component-methods.md` L387 的 C-6 列對內依賴已改為「C-4（HTTP）、C-5、**C-8（型別，建置期）**」，與 `component-dependency.md` L78 依賴矩陣的 C-6×C-8 格（「型別（建置期）」）一致 |
| 6 | Minor | **已修正（圖／說明對齊），但衍生三處計數未同步** | `component-dependency.md` L47-50 四條建置期邊**全部改為 `-.->`**，L59 的說明亦同步為「這四條邊」，圖與文字不再矛盾。**但同一輪未掃到三處連帶敘述**——見 V-2（Minor） |
| 7 | Minor | **已修正** | `decisions.md` L389 已改寫為「**A 沒有實作缺陷，但有一個已記載的能力缺口**」，並明寫「回退即等於重新接受該缺口」；L416 的 Reversibility 另立一段「機械成本不是回退的全部代價」，與 Context 段不再自相矛盾 |
| 8 | Minor | **已修正** | 三處論據全部改為本站實測：`components.md` L328-329（含「不得引用 import smoke」的明文）、`decisions.md` L357-358、`application-design-questions.md` L119。規格檔量級已補記於 `components.md` L358 與 `decisions.md` L379（約 66 KB／36 個 path）。**本輪覆核該數字**：實跑 dump 得 66052 bytes、36 paths、28 schemas、OpenAPI 3.1.0，與記載一致 |

### Critical 2 釘選充分性實測（本輪新跑）

問題：只釘 web 框架與資料驗證函式庫是否足夠？**實測結論：對本 app 足夠。**方法為固定原始碼、改變單一變因後重新 dump 並比對 sha256。

| 變因 | 設定 | sha256 | 判定 |
|---|---|---|---|
| 基準（Python 3.13.7） | 兩支釘住於現行解析版本 | `557fe84c…` | 基準 |
| **Web 應用底層框架版本** | 由 `>=` 下限（0.46.0）拉到現行（1.6.0），跨主版本 | `557fe84c…` | **無影響** |
| **型別輔助套件四支** | `typing-extensions`／`typing-inspection`／`annotated-types`／`annotated-doc` 全部降到各自宣告的最低版 | `557fe84c…` | **無影響** |
| **Python 直譯器版本** | 3.12（CI 使用）vs 3.13（本機預設） | `557fe84c…` | **無影響** |
| **正向對照組** | 兩支釘選函式庫降版 | `ea64b74f…`（20 行差異） | **有影響**（證明檢測方法敏感，前四列的「相同」不是假陰性） |

三項支撐這個結論的結構事實：①資料驗證函式庫對其原生核心是**精確釘選**（metadata 實查為 `==` 形式），故不需另外釘；②Web 框架對底層 ASGI 框架只宣告下限、無上限，**是真的會浮動**，但實測對規格輸出無作用；③本 app 的請求／回應模型**未使用電子郵件型別、表單或檔案上傳**（全樹查無 `EmailStr`／`Form(`／`File(`／`UploadFile`），故 `[standard]` extras 中兩個會影響 schema 的可選套件在本 app 上不成立。

**因此 Q7=A 不需擴大釘選範圍**，這點與委派時的疑慮相反，但有實測支撐。

### 新發現

| # | 嚴重度 | 檔案 | 問題 | 建議修正 |
|---|---|---|---|---|
| V-1 | **Major** | `services.md` L130-131、L136②、L139；`components.md` L310、L313、L348；`component-methods.md` L356、L358 | **型別檔改為 committed 之後，沒有任何機制保證它與規格檔同步，而三處主張聲稱有。**Q6=A 把型別檔從「每次建置由規格重新產生」改成「commit 的產出物」，於是 repo 內有**兩份** committed 產出物，但設計只為其中一份定了 gate：漂移檢查的契約逐字是「重新 dump 並與 commit 的**規格檔**比對」（`components.md` L302、`component-methods.md` L339），**與型別檔無關**。失敗路徑：後端改欄位名 → 開發者重新 dump 規格（否則 backend job 紅燈，有強制力）→ **忘了重跑型別產生** → 型別檔仍宣告舊欄位名 → `tsc -b` 對舊型別檢查前端用法，**通過** → 前端執行期靜默取得 undefined。這正是 AD-9 存在的理由，而它只被開發者自律擋著——也就是 AD-9 否決方案 A 時所拒絕的那個形狀。<br><br>三處主張與此不符：①`services.md` L139 寫「規格檔**或型別檔**未隨程式碼更新 → CI backend job 紅燈」——backend job 是 checkout + setup-python + `pip install` + import smoke + `unittest`（`ci.yml`），**無 node、無 npm、無 frontend devDependency**，結構上不可能重產型別檔，此句所述的攔截點不存在；②`components.md` L348 宣告 C-8 擁有「規格檔與型別檔的產生與**時效性**」，型別檔的時效性無承載；③L313／`component-methods.md` L358 的「編譯期保護不減——`tsc -b` 對 commit 的型別檔檢查」——`tsc -b` 檢查的是**用法對型別檔**，不是**型別檔對規格檔**；型別檔一旦過期，`tsc -b` 只會很順利地通過。<br><br>另有一處直接矛盾：`components.md` L310 與 `component-methods.md` L356 都寫「型別產生只在**開發與 CI** 執行」，但 `services.md` L130 的 CI 影響表把 frontend job 記為「**無新增步驟**」、L131 的 backend job 只加規格漂移檢查、L136② 更明寫「前端完全不碰規格檔」。照 CI 影響表實作，**沒有任何 job 會產生型別**；照 C-8 內文實作，CI 要產。開發者無從判斷該信哪一份 | 二擇一並讓四份 artifact 一致：①**補上型別檔的 gate**——在 frontend job（該 job 為全樹 checkout，可經 `../` 讀到 repo 根的規格檔）新增「重新產生型別並比對，不一致即失敗」一步，同步把 `services.md` L130 的「無新增步驟」改掉、L139 的「或型別檔 → backend job 紅燈」改為 frontend job；②**如實承認沒有 gate**——比照 iteration 3 M2 對 NFR-4 涵蓋邊界的處理方式，把「型別檔的時效性靠開發者自律，無自動化攔截」寫成 C-8 的已知限制，並**同時撤回** L313／L358 的「保護不減」與 L348 的「時效性」宣稱（改為「規格檔的時效性」）。無論選哪個，L310／L356 的「只在開發與 CI 執行」都必須與 CI 影響表對齊 |
| V-2 | Minor | `component-dependency.md` L55、L57、L105 | **Minor 6 的修正把建置期邊由三條拆成四條，但三處連帶敘述未同步。**①L57 仍寫「**十四條**依賴」——該數字對應首版的 11 條執行期 + 3 條建置期；現行圖為 11 實線 + 4 虛線 = **15 條**（且該句實際只列舉了 11 條執行期邊）；②L105 通訊模式表仍為「C-4 → C-8、C-8 → 規格檔、**規格檔 → C-6**…生產環境跑起來後**這三條邊**都不存在」——現行圖是 C-8 → 規格檔 → **型別檔** → C-6，共四條，且「規格檔 → C-6」這條邊已不存在；③L55 文字 fallback 開頭仍寫「分**四**層」，但圖已有五個 subgraph（BUILD 於 L59 另段描述，非未涵蓋，但層數陳述已不準）。三處都是同一次修訂的連帶未掃 | L57 改為「十五條依賴（其中四條為建置期）」或把句子明確限定為「執行期十一條」；L105 改為「C-4 → C-8、C-8 → 規格檔、規格檔 → 型別檔、型別檔 → C-6…這四條邊」；L55 改為「分五層」 |
| V-3 | Minor | `services.md` L27、L29；`components.md` L398 | **Q6=A 的定案未掃到兩張既有表。**①`services.md` L27 的既有服務拓樸表，前端列仍寫「管理頁新增一欄與小螢幕卡片佈局；**建置新增型別產生步驟**」、變更性質仍為「**建置期新增步驟（Revision 1）**」——這是首版的框架；Q6=A 之後前端拿到的是**一份 commit 的型別檔**，不是一個建置步驟，且同檔 L130 自己已寫「無新增步驟」。同表 L29 的「建置與 CI」列只列「repo 新增 committed 規格檔」，**漏了 committed 型別檔**，而 AD-9 自己（L379）與 C-8 已知代價（`components.md` L358）都強調是「**兩份**」產出物。②`components.md` L398 的 NFR-3 列說明仍是「四面向的逐項判定已在 requirements 完成；本站的落點是權限變更本身」——Revision 1 之後本站多了第二個 security 落點（C-8 的 network exposure 明文契約，`decisions.md` L407），只讀此表的人不會知道 | L27 改為「管理頁新增一欄與小螢幕卡片佈局；新增一份 commit 的型別檔（**建置流程不變**）」；L29 補上型別檔；L398 承載元件補「C-8（network exposure 面向）」並指向 `decisions.md` AD-9 的四面向表 |
| V-4 | Minor | `components.md` L344；`component-methods.md` L360-364；`decisions.md` L381 | **釘選的兩項實作細節未定，兩者都會讓 Q7=A 的效力打折。**①**釘選形式未寫明**：三處都只說「釘住版本」，未指明必須是**精確等值**。以相容釋出形式（`~=`）釘在次版本線上仍會浮動，而本輪正向對照組的 20 行差異正是跨次版本產生的——形式選錯等於沒釘。②**升版時的連帶義務未寫**：釘住之後，任何一次刻意升版都會讓漂移檢查在**該次升版 PR** 上變紅（因為規格輸出真的變了）。這是正確行為，但設計花了大量篇幅論證「這道 gate 不得誤報」，卻沒說明「升版 PR 必須在同一個 PR 內重新 dump 規格檔」——不寫下來，第一次升版時會被當成誤報 | C-8 的第四項契約要求補兩句：「釘選必須為精確等值形式」與「升版這兩支依賴時，必須在同一個 PR 內重新 dump 規格檔並一併 commit」 |
| V-5 | Minor | `components.md` L297-302（C-8 四個組成表）；`component-methods.md` L332-339（產物契約表） | **型別檔進入 `frontend/src/` 之後會被既有 lint 掃到，設計未指明它是受檢還是排除。**實測：`frontend/eslint.config.js` 的忽略清單只有 `globalIgnores(['dist'])`，套用範圍為 `files: ['**/*.{ts,tsx}']`——**`frontend/src/` 下的任何 `.ts`／`.d.ts` 都在 `npm run lint` 的作用域內**，且套用 `tseslint.configs.recommended`（含數條 error 級規則）。產生器輸出的型別檔若觸發其中任一條 error，`npm run lint` 即紅燈，而 team-practices 已記載 CI 只擋 error、現況為 0 error。這與 Major 4 已處理的 `package-lock.json` 同步屬同一類：新 committed 產出物穿過的既有 gate 未被盤點。爆炸半徑小、失敗大聲（不是靜默），故為 Minor | 在 C-8 產物契約補一列：型別檔**是否納入 lint 作用域**須於採用時定案（納入則產生器輸出必須自然通過現行規則集；排除則需在 `eslint.config.js` 的忽略清單加入該路徑，並記明這是刻意的產出物豁免） |

### Summary

**8 條 findings 全部處理到位，兩個 Critical 的定案都經回 repo 與實跑驗證後站得住。**Critical 1 的推論逐段成立——`.dockerignore` 不排除 `src/` 內任何路徑、三處 build context 複核無誤、規格檔留在 repo 根目錄既不在 frontend 也不在 backend 的映像 context 內；本輪另補查的兩條路徑（`lint-fix` 的 `npm run build`、`ui-regression` 的 compose 建置）也不構成漏洞。Critical 2 的釘選範圍**經實測足夠**，這一點與「只釘兩支可能不夠」的直覺相反：底層 ASGI 框架確實在浮動（上游只宣告下限、無上限，現行已跨到主版本 1.x），但跨 0.46.0→1.6.0 的輸出**位元相同**；型別輔助套件四支全降到宣告最低版、Python 由 3.13 換到 CI 用的 3.12，輸出同樣位元相同；正向對照組（兩支釘選函式庫降版）則如實產生 20 行差異與不同 sha256，證明檢測方法本身是敏感的。Major 3 的四面向表四項齊全且不適用者皆附理由，network exposure 的處置經 nginx 與 router 掛載點複核**確實擋住風險**。Major 4 的第四項契約要求可核對，且與現行 tsconfig 的設定相容。

**唯一的 Major 是 Q6=A 這個修正本身新開的洞**：型別檔從「每次建置重新產生」變成「commit 的產出物」之後，repo 內有兩份 committed 產出物，但漂移檢查只管規格檔那一份。型別檔過期時，唯一被聲稱的攔截點（`tsc -b`）在結構上不可能發現——它檢查的是用法對型別檔，不是型別檔對規格檔。而三處主張聲稱有攔截：`services.md` L139 把「型別檔未更新」記為 backend job 紅燈（該 job 無 node，結構上不可能重產型別），`components.md` L348 宣告 C-8 擁有型別檔的「時效性」，L313／`component-methods.md` L358 宣稱「保護不減」。更外顯的是 C-8 內文（「型別產生只在開發與 CI 執行」）與 `services.md` CI 影響表（frontend「無新增步驟」、backend 只加規格漂移檢查、「前端完全不碰規格檔」）**互相矛盾**，開發者無從判斷該不該加那一步。修法很便宜——在 frontend job 加一步「重產型別並比對」（該 job 為全樹 checkout，讀得到 repo 根的規格檔），或比照 iteration 3 M2 對 NFR-4 涵蓋邊界的做法如實承認沒有 gate 並撤回上述三處宣稱。它不阻擋 READY：本 intent 的兩個欄位在首次產生時型別必然是新的，這條失敗路徑要到**日後某次後端 API 變更**才會實現。

依 verdict 規則（0 Critical、1 Major）判定 **READY**。開發者依這五份文件可以實作出正確的東西；V-1 需要一次二選一的定案，但兩個選項都寫在建議欄裡，不需回頭問架構師。

**帶入 Construction 的殘留事項**（接續 iteration 4 Summary 已列的 6 項）：

7. **V-1（Major）** —— 型別檔時效性：補 gate（frontend job 重產並比對）或如實記為已知限制並撤回三處宣稱；`components.md` L310／`component-methods.md` L356 與 `services.md` CI 影響表必須對齊。
8. **V-4（Minor）** —— 釘選必須為精確等值形式；升版該兩支依賴時須在同一個 PR 內重新 dump 規格檔。
9. **V-5（Minor）** —— 型別檔是否納入 `npm run lint` 作用域須於採用時定案（納入則須自然通過現行規則集，排除則需在忽略清單明文加入）。
10. **V-2／V-3（Minor）** —— `component-dependency.md` 的邊數與層數敘述、`services.md` 拓樸表的前端列與產出物列、`components.md` NFR-3 列，四處與 Q6=A 定案同步。
11. **規格檔的檔名與確切路徑本站未定** —— 留給 Construction，但須同時確認：不觸犯 `scripts/validate_repo_contract.py` 的禁止路徑規則（本輪實跑該腳本為 PASS，作為變更前的基準），且不落在 `frontend/public/`（C-8 明文契約）。

---

## Review — Revision 1

**Reviewer**: aidlc-architecture-reviewer-agent · Revision 1
**Date**: 2026-08-09T05:55:56Z
**審查範圍**：僅 Revision 1 新增／修改的部分（Q4／Q5、AD-9、C-8、C-6 型別來源、`services.md` 建置與 CI 段、`component-dependency.md` 的 BUILD 子圖與矩陣）。iteration 1〜4 已驗收的 C-1〜C-7、AD-1〜AD-8 不重審。
**Verdict**: **NOT-READY**（2 Critical）

### 事實主張查證

| 主張 | 查證結果 | 判定 |
|---|---|---|
| CI 的 frontend job 是獨立的，後端不會在該 job 中運行 | `.github/workflows/ci.yml:37-59`：`frontend` job 步驟為 checkout → setup-node → `npm ci` → `npm run lint` → `npm run build`。無 Python、無後端服務、無 service container。該 job 取得 live 規格的唯一途徑不存在 | **成立** |
| frontend build = `tsc -b && vite build` | `frontend/package.json:8` 逐字為 `"build": "tsc -b && vite build"`；`ci.yml:57` 的註解亦同 | **成立** |
| 前端有 52 處 `fetch`、10 支檔 | 實測 `frontend/src` 下 `*.ts`／`*.tsx`：**52 處、10 支檔**（AssessmentPage 16、WorkspacePage 13、LensCriteriaEditor 4、AdminPage 4、RolePermissionsPage 4、ShareModal 3、AuthorizationRequestsPage 3、LoginPage 2、WaitingApprovalPage 2、AuthContext 1） | **成立（數字精確）** |
| backend job 的 import smoke 證實規格可在不啟動服務、不連資料庫的前提下取得 | 結論**成立**，但**所述證據不支持該結論**。`ci.yml:85` 只跑 `import main; main.app.title`，從未呼叫 `app.openapi()`；「可 import」不蘊含「可產生規格」（規格產生會對全部 36 個 path 與 28 個 schema 走一次 JSON Schema 生成，是全新且從未在 CI 上執行過的碼路徑）。reviewer 另行實測補上該證據：以獨立 venv 實際執行 `import main; main.app.openapi()`，**成功**，產出 36 paths／28 schemas／OpenAPI 3.1.0（約 66 KB）。`database.py:19-23` 的 `DATABASE_URL` 有預設值且 `create_engine` 為惰性、`services/auth.py:13` 的 `JWT_SECRET` 有預設值，故連環境變數都非必要 | **結論成立，論據無效**（見 Finding 8） |
| C-8 完全在建置期、不進入生產環境的執行路徑 | 對「TypeScript 型別」而言成立（型別於編譯期抹除）。但**此不變式由工具選擇決定，而設計未把它寫成契約** —— 產生 runtime client／zod validator 的產生器同樣滿足 C-8 現有的三項契約要求，卻直接推翻此宣稱 | **有條件成立；缺欠承載**（見 Finding 4） |
| 漂移檢查（重新 dump 並比對）在 CI 上能穩定運作 | **分兩層，結論相反**。①**同一組依賴版本下輸出是位元決定性的**：以 `PYTHONHASHSEED` = 0／1／42／12345／999983 各 dump 一次，五份 sha256 完全相同（`557fe84c…`）—— 逐檔核對後確認 36 個 path 中有 7 個多方法 path，其方法鍵序來自 route 註冊序而非 set 迭代序，故 hash 隨機化不影響輸出。**「每次執行都可能不同」的假設不成立**。②**跨依賴版本則不穩定**：同一份原始碼，僅把 `fastapi` 0.141.1／`pydantic` 2.13.4 換成 0.115.0／2.9.2，輸出即出現 20 行差異（`additionalProperties` 增刪、`ValidationError` 的 `input`／`ctx` 欄位增刪），sha256 由 `557fe84c…` 變為 `ea64b74f…` | **不成立**（見 Finding 2） |

### Findings

| # | 嚴重度 | 檔案 | 問題 | 建議修正 |
|---|---|---|---|---|
| 1 | **Critical** | `services.md`「建置與 CI 的影響」表；`components.md` C-8；`component-methods.md` C-8 產物契約 | **規格檔的存放位置未指定，而 docker-build 與 staging 部署會因此失敗。**設計說型別產生步驟「接在既有 `tsc -b && vite build` 之前」（即納入 `npm run build`），同時在 CI 影響表把 `docker-build` 記為「無」變更。實測：`frontend/Dockerfile` 為 `COPY . .` + `RUN npm run build`，而**三條建置路徑的 context 都只有 `frontend/` 目錄** —— `ci.yml:104-119`（`context: ./frontend`）、`deploy/docker-compose.deploy.yml:53`（`context: ../frontend`）、`deploy/docker-compose.test.yml:46`（同）。規格檔由**後端**腳本產生，最自然的落點是 repo 根目錄（本 repo 既有先例：`schema.sql`／`schema_rbac.sql`／`DEPLOY.md` 皆在根目錄）或 `backend/`，兩者都**不在 frontend 的 build context 內**，映像建置時讀不到 → CI 第四道 gate 紅燈、`deploy.yml:119` 的 `docker compose up -d --build` 失敗（進而觸發 rollback job 開 revert PR）、`ui-regression` 的短生命週期 stack 起不來。設計不但未指定位置，還以「docker-build：無」主動告訴讀者不必考慮這件事 | 三選一並寫進 C-8 產物契約：①明訂規格檔位於 `frontend/` 內（由後端腳本跨目錄寫入，並在 C-8 記明此為刻意的跨層寫入）；②改為**產生的型別檔一併 commit**、型別產生只在開發與 CI 跑而不進 `npm run build`（則 Docker 完全不需要規格檔，`docker-build` 才真的「無」變更）；③把 Dockerfile 的 build context 改為 repo 根目錄（成本最高，影響三處編排檔與 `.dockerignore`）。無論選哪個，`services.md` 的 CI 影響表都必須更正 `docker-build` 那一列 |
| 2 | **Critical** | `decisions.md` AD-9「Consequences／負面」；`components.md` C-8；`services.md`「新增的失敗模式」 | **漂移檢查會在完全沒有漂移的 PR 上變紅，設計的成本分析漏記此失敗模式。**實測（見上表）：規格輸出在固定依賴版本下是位元決定性的，但**跨版本不是** —— 僅換 fastapi／pydantic 版本，同一份原始碼產出的規格即有 20 行差異與不同 sha256。而 `backend/requirements.txt` **12 行依賴無一有版本約束、無 lockfile**（`team.md ## Code Style` 已如實記載「CI／Docker build／staging 部署三處各自解析當下最新版，可能彼此不同」），CI 的 backend job 每次跑 `pip install -r requirements.txt` 都重新向索引解析最新版。因此：commit 規格檔之後，只要 FastAPI 或 Pydantic 發一次新版，下一個**完全無關**的 PR 就會在漂移檢查上紅燈；且該紅燈與「真的漂移了」在訊號上不可區分，而標準補救（本機重新 dump）在開發者本機解析到第三個版本時不會收斂。AD-9 的負面清單只寫了「忘了重新 dump 就是 CI 紅燈」，**未預見「沒忘記也會紅燈」**——這正是這道 gate 被論證為值得引入的成本基礎 | 把「規格產生所依賴的版本必須釘住」升格為 C-8 的第四項契約要求，並在 AD-9 的 Consequences 補記此失敗模式。可行形狀（擇一，記入設計而非留給實作猜）：①釘住 `fastapi`／`pydantic` 版本（但這會與 `team.md` 記載的「全 repo 未 pin」現況形成局部例外，需明記理由）；②漂移檢查改為對**規格中與前端契約相關的子集**（例如僅 `components.schemas.UserSchema`）比對，使其對函式庫版本雜訊免疫；③把版本釘住列為 C-8 的**上線前置依賴**並登錄於 RAID／backlog。若三者皆不做，則此 gate 依設計必然是 flaky gate，應在 AD-9 中如實標示而非略過 |
| 3 | **Major** | `decisions.md` AD-9（全篇）；`components.md` C-8、非功能需求表 NFR-3 列 | **Revision 1 未對 AD-9／C-8 做 ADR-0006 四面向判定，而其中一面向有具體風險。**`project.md ## Mandated` 逐字要求「對每一項變更檢查 ADR-0006 security baseline 的四個面向（IAM、encryption、network exposure、audit logging）…須在該 stage 產出中明列 security 影響與處置，不得僅以『已有 ADR-0006』帶過」；`phases/inception.md` 亦要求「Security and compliance implications must be addressed for every major architectural decision」；`project.md ## Corrections`（`requirements-analysis:c4`）要求此類「缺一不可」型約束以逐項判定表呈現、不適用者一律附理由。AD-9 是本 intent 唯一具工具鏈黏性的架構決策，卻**零 security 段落**，NFR-3 列也未更新。**network exposure 面向有實據**：`frontend/nginx.conf` 只反向代理 `/api/`，後端自帶的 `/openapi.json` 與 `/docs` 目前**在公網上不可達**；而 Vite 預設會把 `frontend/public/` 原樣複製進 `dist/`，nginx 的 `location /` 以 `try_files $uri` 直接供檔 —— 一旦規格檔落在 `frontend/public/`（Finding 1 的壓力正是把檔案往 `frontend/` 推），`https://cloud360.danniel.cc/<spec>.json` 就會把 36 個 path、28 個 schema 的完整 API 地圖（含全部使用者管理與權限端點）對未認證訪客公開，且 nginx 已對 `application/json` 開 gzip | 在 AD-9 補一節 ADR-0006 四面向逐項判定表（IAM／encryption／network exposure／audit logging，不適用者附理由）；並把「規格檔不得落在任何會被靜態服務原樣供出的目錄（含 `frontend/public/`）」寫成 C-8 的明文契約，與 Finding 1 的位置定案一併決定 |
| 4 | **Major** | `component-methods.md` C-8「三項契約要求」；`components.md` C-8「邊界與所有權」 | **「不進入生產環境的執行路徑」是 AD-9 得以成為 AD-5 窄例外的全部依據，卻沒有任何契約守住它。**C-8 現有的三項契約要求是「規格取得不依賴執行中的服務」「漂移檢查必須能失敗」「採用範圍限於使用者物件型別」—— 一個產生 runtime API client 或 zod 執行期驗證器的產生器**三項全部滿足**，卻直接推翻該不變式，並使 `services.md`「下方運行特性與失敗模式各節因此完全不受影響」的論證失效。設計既未指名工具，也未限定工具類別 | 新增第四項契約要求：「產生器只得輸出型別宣告，不得輸出任何進入 bundle 的執行期程式碼（client、fetch wrapper、驗證器）」，並要求在採用時以可核對的方式驗證（例如產出物僅為 `.d.ts`／`type`-only 匯出）。同時補記 `frontend/package-lock.json` 須隨新 devDependency 一併重產並 commit（CI 用 `npm ci`，lock 與 `package.json` 不一致即硬失敗） |
| 5 | Minor | `component-methods.md`「介面契約摘要」表 | C-6 那一列的「對內依賴」仍是「C-4（HTTP）、C-5」，未加入 Revision 1 新建立的 C-6→C-8（建置期型別）依賴；同一次修訂的 `component-dependency.md` 依賴矩陣**已**記載該邊。兩份 artifact 在同一輪修訂後互相矛盾 | C-6 列補上「C-8（型別，建置期）」 |
| 6 | Minor | `component-dependency.md` 依賴圖與其下說明 | 說明寫「圖中的虛線邊全部發生在建置期 —— C-4 的回應模型即規格內容、C-8 dump 出規格檔、規格檔產生前端型別」，列舉三條為虛線；但 Mermaid 中 `C8 -->|"dump"| SPEC` 是**實線**，只有兩條是 `-.->`。圖與文字對不上，而「虛線＝建置期」正是本圖此輪新增的閱讀約定 | 把 `C8 --> SPEC` 改為 `C8 -.-> SPEC`，或改寫說明為「BUILD 子圖內與跨進／跨出該子圖的三條邊皆為建置期」 |
| 7 | Minor | `decisions.md` AD-9「Alternatives Rejected」A 項與「Reversibility」 | 「此方案本身沒有缺陷 —— 它被否決是因為它不提供所要的保證，不是因為它有問題」與同一則 ADR 的 Context 段「這個形狀有一個**已知且已記載的缺口**」自相矛盾（`phases/inception.md` 要求不得帶著未解的矛盾前行）。此措辭並非中性：AD-9 同時把 A 指為回退目標，而 Reversibility 只算了移除工具鏈的機械成本，**未記回退即等於重新接受該缺口** | 改寫為「A 沒有實作缺陷，但有已記載的能力缺口（前端無編譯期保護）」，並在 Reversibility 補一句「回退至 A 即重新接受該缺口，這是回退的真實代價之一」 |
| 8 | Minor | `decisions.md` AD-9 Context 第 2 點；`components.md` C-8「為何規格來自檔案而非 live 端點」；`application-design-questions.md` Q4 前言 | 三處以相同措辭主張「backend job 已在跑 import smoke，**證實**規格可在不啟動服務、不連資料庫的前提下取得」。import smoke 只證實 `import main` 成功，未曾呼叫 `app.openapi()`；規格產生是從未在 CI 上執行過的新碼路徑。結論經 reviewer 實測為真（36 paths／28 schemas／3.1.0，且不需環境變數），但**原論據不成立**。另：規格檔約 66 KB、涵蓋 36 個 path，此體量在任何 artifact 中皆未記載，而它是每次後端 API 變更都會出現在 diff 中的 committed 產出物 | 把論據改為「已由本站實測 `app.openapi()` 於無服務、無資料庫下可取得（36 paths／28 schemas）」，並在 C-8 的已知代價補記規格檔的量級與其對 PR diff 的影響 |

### 修訂協定合規性

| 檢查項 | 結果 |
|---|---|
| AD-5 原文是否被改寫 | **未被改寫**。逐行 diff（`archive/2026-08-09-application-design/decisions.md` vs 現行）顯示 AD-5 區段只在第 203 行後 append 一則具名的交叉引用註記，Decision／Consequences／Alternatives／Reversibility 全部逐字不動 |
| AD-1〜AD-8 是否被動過 | **未動**。decisions.md 的 diff 僅四處：檔頭版本行、定案分類清單新增一行、AD-5 後的註記、AD-9 全段與對應表新增一列 |
| Q1〜Q3 與其答案是否被動過 | **無跡象被動**：Q4／Q5 以 `## Revision 1` 分隔後 append，Q1〜Q3 結構與 `[Answer]: A` 完整。惟問題檔未納入歸檔且 record 尚未進版控（`inception/` 全樹為 untracked），故此項**無可比對的基準**。此與 `archive/2026-08-04-scope-definition`（同樣只歸檔兩份 artifact、不歸檔問題檔）的既有先例一致，不另計為 finding |
| 歸檔完整性 | **完整**。五份 artifact 全數歸檔於 `archive/2026-08-09-application-design/`，且各檔 diff 的 hunk 皆為 Revision 1 描述範圍內的小幅增修，無夾帶 |
| 既有審查紀錄是否被動 | **未動**。components.md 的 diff 最後一個 hunk 在第 332 行（約束表），`## Review — Iteration 1〜4` 四段全部位元相同 |
| `services.md` 結論先行的改寫 | 屬**必要且已標示**的修訂：原句「不新增外部依賴」在 AD-9 後已為偽，改寫並以具名的 Revision 1 區塊說明範圍界線，符合 `project.md ## Corrections`（`refined-mockups:c3`）對「對齊修正須明記」的要求 |

### Summary

修訂協定執行得乾淨 —— AD-5 原文完整保留、既有問答與四輪審查紀錄未被動、歸檔完整，AD-9 對自身代價（devDependency、規格檔維護、可逆性中等）的記載也誠實。**擋下的是兩件實測層面的事**：其一，型別產生步驟被放進 `npm run build`，而前端映像的 build context 只有 `frontend/` 目錄，規格檔位置未指定即代表 `docker-build` gate 與 staging 部署有一半機率直接壞掉，而設計反倒宣稱 `docker-build` 不受影響；其二，漂移檢查的穩定性判斷需要修正方向 —— 它不是每次執行都飄（實測五組 hash seed 位元相同），而是**跟著未釘住的依賴版本飄**（實測換版即 20 行差異），在一個 12 行依賴全無版本約束、無 lockfile 的 repo 上，這道新 gate 註定會在無關的 PR 上變紅，而 AD-9 的成本分析沒有這一條。另有一項 Major 是規則層的硬性缺漏：`project.md ## Mandated` 要求每一項變更逐項判定 ADR-0006 四面向，AD-9 完全沒做，而其中的 network exposure 面向在此案有具體風險（規格檔若落在 `frontend/public/`，完整 API 地圖會經 nginx 對公網供出）。這三項都落在「新引入工具鏈」這一塊，與前四輪已驗收的 C-1〜C-7 無關。
...
本節新增 **C-9**，並擴充 **C-4** 與 **C-6**。C-1、C-2、C-3、C-5、C-7、C-8 的職責與邊界**完全不變**。

### 元件清單（Revision 1 追加）

| ID | 元件 | 所在層 | 新增／既有 | 核心職責 |
|---|---|---|---|---|
| C-9 | 使用者清單分頁 | 跨層（後端 API ＋ 前端頁面／元件） | 新增（Revision 1） | 清單端點的分頁查詢與 envelope 回應；分頁控制的呈現與切頁；跨頁一致性的收斂 |

### C-4 使用者物件序列化（Revision 1 擴充）

**新增職責**：清單端點的回應由 `List[UserSchema]` 改為具名 envelope（AD-10）。

**三個構造點的既有約束完全不變** —— envelope 只是把 `items` 包起來，逐列的 `UserSchema` 構造仍在同一處，兩個新欄位仍**不得設可靜默通過的預設值**。

> **本 intent 最容易被忽略的一點**：envelope 是**第四個**回應構造點。前三個（清單、啟停用、角色調整）是 `UserSchema` 的構造點；envelope 是 `UserListPage` 的構造點，它的 `total`／`page`／`page_size` 三個欄位同樣可能因為「回應模型帶了預設值」而在完全沒讀查詢參數的實作下照樣輸出 —— 這正是 AC-5.2 被改寫為值斷言（而非存在性斷言）的原因。**envelope 的三個分頁欄位同樣不得設可靜默通過的預設值。**

**邊界不變**：C-4 不擁有分頁的查詢邏輯（屬 C-9），只擁有回應形狀與逐列欄位的一致性。

### C-6 管理頁資料傳遞（Revision 1 擴充）

**新增職責**：持有分頁狀態（目前頁次）、把 envelope 拆解為 `items` 與三個分頁值、把後者傳給 C-9 的前端部分、在切頁時觸發抓取。

**既有約束不變**：資料抓取仍須遵循既有的三層拆分（純抓取函式／呼叫端更新 state／`useEffect` 內 `cancelled` flag），逾期旗標仍由後端傳入、不在渲染階段計算。

**新增約束**：分頁切換是 `fetchUserList` 的**第三個呼叫點**（既有兩個：`useEffect` 內、`fetchUsers` 內）。逾期旗標的正規化若寫在呼叫端而非單一抓取點，**新增的切頁路徑會漏套用**，使該頁整頁不帶標示 —— AC-5.8 正是為此改寫落點。**正規化必須收斂在抓取函式內，不得由三個呼叫端各自處理。**

### C-9 使用者清單分頁

**職責**：讓使用者清單以頁為單位取得與呈現，且切頁與頁面內處置都不損害逐帳號查驗的工作流。

**後端部分**：

- 清單端點接受兩個查詢參數（頁次、每頁筆數），以**框架原生的範圍約束**宣告（AD-11）—— 非法值在進入處理函式之前被擋下，結構上到不了查詢層。
- 以 offset 分頁查詢，**保留既有的 `ORDER BY id` 全序**（AC-5.3 的結構前提，不得移除）。
- 回應為具名 envelope，四個欄位皆為必填、**皆不得設可靜默通過的預設值**（AD-10、AC-5.2）。
- 超出範圍的頁次：回 200＋空 `items`，`page` 回顯請求值不夾頁（FR-6.4）。

**前端部分**：

- `PaginationControl` 元件，六個狀態與 props 契約見 `refined-mockups/interaction-spec.md`。**不自行抓取資料**、**不自行判斷忙碌狀態**（皆由呼叫端傳入，與 `LastActivityCell` 的 `isOverdue` 同一形狀，理由亦同：渲染純度 lint 規則）。
- 渲染位置在表格／卡片容器**之外**（refined-mockups 定案 5）—— 這是 AC-5.10 的結構前提，不是樣式偏好。
- 刪除後的收斂：就地移除該列＋總筆數本地遞減，接著以目前頁次重抓該頁（AD-12）。啟停用與角色調整純就地更新、不重抓。

**邊界與所有權**：

- **擁有**：分頁查詢參數的契約與驗證、offset 查詢、envelope 的分頁三值、分頁控制的呈現與切頁、跨頁一致性的收斂策略
- **不擁有**：逐列使用者物件的欄位內容（屬 C-4）、逾期判定（屬 C-1）、儲存格呈現（屬 C-5）、型別檔的產生（屬 C-8）

**與 C-8 的關係**：envelope 是具名 schema，因此 `total`／`page`／`page_size` 會出現在產生的型別中。這是 AD-10 選 envelope 而非回應標頭的決定性理由 —— 標頭不進入回應 schema，等於讓 C-8 對這三個值零保護。

**驗證落點**：

| 面向 | 驗證方式 | 能否真的失敗 |
|---|---|---|
| 分頁查詢與 envelope 三值 | `TestClient` 端點測試（值斷言，非存在性） | ✅ |
| 非法參數不進查詢層 | `TestClient` 斷言 422 且不回傳帳號資料 | ✅ |
| 超出範圍回 200＋空＋頁次回顯 | `TestClient` | ✅ |
| 切頁不重複、含刪除時無遺漏 | Playwright e2e（需以公開註冊端點造超過一頁的帳號） | ✅ |
| 處置後維持頁次 | Playwright e2e | ✅（`:113`／`:129` 現行整份重抓，照現行形狀實作必然失敗） |
| 分頁控制的無障礙 | 部分 e2e、部分人工（見 `accessibility-checklist.md` Revision 1） | 部分 |

### 元件與需求的對應（Revision 1 追加）

| 需求 | 承載元件 |
|---|---|
| FR-6.1、FR-6.2 | C-9（後端）、C-4（envelope 形狀） |
| FR-6.3 | C-9（前端）、C-6 |
| FR-6.4 | C-9（後端回應）、C-9（前端空態）、C-6 |
| FR-6.5 | C-9（前端，AD-12）、C-6 |
| FR-6.6 | C-1（判定不變）、C-4（值一致） |
| FR-6.7 | C-9（後端 —— 不接受非分頁查詢參數） |
| NFR-8 | C-9（後端，AD-11） |
| NFR-9 | C-9（前端） |
| NFR-10 | C-9（後端）的測試義務 |
